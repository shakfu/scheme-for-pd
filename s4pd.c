...
void s4pd_eval_atoms_as_string(t_s4pd* x, t_symbol* s, long argc,
                               t_atom* argv);
...
{
    s4pd_class = class_new(gensym("s4pd"), (t_newmethod)s4pd_new,
                           (t_method)s4pd_free, sizeof(t_s4pd), CLASS_DEFAULT,
                           A_GIMME, // allow dynamic number of arguments
                           0);
    class_addmethod(s4pd_class, (t_method)s4pd_log_null, gensym("log-null"),
                    A_DEFFLOAT, 0);
    class_addmethod(s4pd_class, (t_method)s4pd_log_repl, gensym("log-repl"),
                    A_DEFFLOAT, 0);
    class_addmethod(s4pd_class, (t_method)s4pd_reset, gensym("reset"), 0);
    class_addmethod(s4pd_class, (t_method)s4pd_cancel_clocks,
                    gensym("cancel-clocks"), 0);
    class_addmethod(s4pd_class, (t_method)s4pd_read, gensym("read"), A_SYMBOL,
                    0);
...
void* s4pd_new(t_symbol* s, int argc, t_atom* argv)
{
    ...
    // post("s4pd_new() outlets: %i filename: %s", x->num_outlets,
    // x->filename->s_name); make the outlets
...
void s4pd_free(t_s4pd* x) { s4pd_cancel_clocks(x); }
...
void s4pd_init_s7(t_s4pd* x)
{
    ...
    s7_define_function(x->s7, "load-from-path", s7_load_from_path, 1, 0, false,
                       "load a file using the search path");
    s7_define_function(x->s7, "out", s7_pd_output, 2, 0, false,
                       "(out 1 99) sends value 99 out outlet 1");
    s7_define_function(x->s7, "post", s7_post, 1, 0, true,
                       "posts output to the console");
    s7_define_function(x->s7, "send", s7_send, 2, 0, true,
                       "sends message to a receiver");

    s7_define_function(x->s7, "table-read", s7_table_read, 2, 0, false,
                       "read a point from an array");
    s7_define_function(x->s7, "tabr", s7_table_read, 2, 0, false,
                       "read a point from an array");
    s7_define_function(x->s7, "table-write", s7_table_write, 3, 0, false,
                       "write a point to an array");
    s7_define_function(x->s7, "tabw", s7_table_write, 3, 0, false,
                       "write a point to an array");

    s7_define_function(x->s7, "s4pd-schedule-delay", s7_schedule_delay, 2, 0,
                       false, "schedule a delay callback");
    s7_define_function(x->s7, "s4pd-cancel-clocks", s7_cancel_clocks, 0, 0,
                       false, "cancel all clocks");
    // not in use right now, might bring it back later
    // s7_define_function(x->s7, "s4pd-cancel-delay", s7_cancel_delay, 1, 0,
    // false, "cancel and free a clock delay");
    ...
    s7_eval_c_string(
        x->s7,
        "(begin (define s4pd-loaded #f) (define s4pd-schedule-loaded #f))");
    ...
    s7_pointer loaded_ok = s7_eval_c_string(
        x->s7, "(and s4pd-loaded s4pd-schedule-loaded)");
...
void s4pd_read(t_s4pd* x, t_symbol* s)
{
    ...
    // use open_via_path to get full path, but then don't load from the
    // descriptor
    int filedesc;
    char path_buf[MAXPDSTRING], *name_buf;
    char load_string[MAXPDSTRING];
    if ((filedesc = canvas_open(x->x_canvas, s->s_name, "", path_buf,
                                &name_buf, MAXPDSTRING, 0))
        < 0) {
        post("s4pd: Error, can't find file %s. (Check Pd file paths)",
             s->s_name);
        return;
    }
    close(filedesc);
...
{
    // get our max object by reading the max pointer from the scheme
    // environment
    uintptr_t s4pd_ptr_from_s7 = (uintptr_t)s7_integer(
        s7_name_to_value(s7, "pd-obj"));
...
s7_pointer atom_to_s7_obj(s7_scheme* s7, t_atom* ap)
{
    ...
    switch (ap->a_type) {
    // Pd floats get turned into s7 reals. not sure how to deal with lack of
    // ints in Pd!
    case A_FLOAT:
        // post(" ... creating s7 real");
        s7_obj = s7_make_real(s7, atom_getfloat(ap));
        break;
    // Pd symbols could be any of: strings, symbols, quoted symbols, symbols
    // for #t or #f
    case A_SYMBOL:
        ...
        if (in_quotes(atom_getsymbol(ap)->s_name)) {
            ...
            // if sent 'foobar, we actually want in s7 the list: ('quote
            // 'foobar) post(" ... creating s7 quoted symbol");
            s7_obj = s7_nil(s7);
            s7_obj = s7_cons(
                s7,
                s7_make_symbol(s7,
                               trim_symbol_quote(atom_getsymbol(ap)->s_name)),
                s7_obj);
...
int s7_obj_to_atom(s7_scheme* s7, s7_pointer* s7_obj, t_atom* atom)
{
    ...
    // TODO: not yet handling lists or vectors (in Max, these become atom
    // arrays)
    ...
    if (s7_is_boolean(s7_obj)) {
        ...
        post("ERROR: unhandled Scheme to Max conversion for: %s",
             s7_object_to_c_string(s7, s7_obj));
        return 1;
    }
    return 0;
}
...
{
    // post("s4pd_reset()");
    //  cancel and free any outstanding delays by calling clear-delays in
    //  scheme
...
void s4pd_message(t_s4pd* x, t_symbol* s, int argc, t_atom* argv)
{
    ...
    // case for code as generic list of atoms with first atom starting with a
    // paren convert the first symbol and the remaining atoms into a string an
    // eval-string it 2021-11-21 discovered this is crashing, disabling for now
    // else if(s->s_name[0] == '('){
    //  s4pd_eval_atoms_as_string(x, s, argc, argv);
    //}
    // case for code as generic list of atoms, no parens
    // we eval as if it was surrounded by parens by building an s7 list and
    // eval'ing
    else {
        t_atom* ap;
...
void s4pd_eval_atoms_as_string(t_s4pd* x, t_symbol* s, long argc, t_atom* argv)
{
    ...
    if (argc == 0) {
        ...
        binbuf_free(code_bb);
        // code_text is now a c string of the code for everything from the
        // first symbol on code_length is the length of the text not including
        // size of first symbol post("code text: '%s'", code_text_from_atoms);
...
static s7_pointer s7_load_from_path(s7_scheme* s7, s7_pointer args)
{
    ...
    char path_buf[MAXPDSTRING], *name_buf;
    char load_string[MAXPDSTRING];
    if ((filedesc = canvas_open(x->x_canvas, filename, "", path_buf, &name_buf,
                                MAXPDSTRING, 0))
        < 0) {
        post("s4pd: Error, can't find file %s. (Check Pd file paths)",
             filename);
...
static s7_pointer s7_send(s7_scheme* s7, s7_pointer args)
{
    ...
    int starting_arg = 2;
    // initialize return value to nil, as we need to return something to S7
    // even on errors
    ...
    if (s7_is_symbol(s7_car(args))) {
        ...
        pd_error((t_object*)x,
                 "s4pd: (send): arg 1 must be a symbol of a receiver name");
    ...
    // NB: in PD & Max, a message "1 2 3" is actually sent internally as "list
    // 1 2 3"
    if (s7_is_symbol(s7_cadr(args))) {
        msg_symbol = s7_symbol_name(s7_cadr(args));
    } else if (s7_is_number(s7_cadr(args))) {
        // if the first arg is a number, figure out if message is one number or
        // a list
        ...
        pd_error(
            (t_object*)x,
            "s4pd: (send): arg 2 should be a symbol of the message to send");
        return s7_return_value;
    }
    // loop through the args to build an atom list of the right length
    // TODO learn how to do this correctly, and add error handling for over the
    // limit yo
    t_atom arg_atoms[MAX_ATOMS_PER_MESSAGE];
    int num_atoms = s7_list_length(s7, args) - starting_arg;
    // build arg list of atoms
    for (int i = 0; i < num_atoms; i++) {
        err = s7_obj_to_atom(s7, s7_list_ref(s7, args, i + starting_arg),
                             arg_atoms + i);
        if (err) {
            pd_error((t_object*)x,
                     "s4pd: (send): error converting Scheme arg to Pd atom, "
                     "aborting");
            return s7_return_value;
        }
    }
    // now we can send the message to the receiver
    //     err = object_method_typed(obj, gensym(msg_symbol), num_atoms,
    //     arg_atoms, NULL);
    typedmess(gensym(receiver_name)->s_thing, gensym(msg_symbol), num_atoms,
              arg_atoms);
    return s7_return_value;
}
...
static s7_pointer s7_table_read(s7_scheme* s7, s7_pointer args)
{
    ...
    if (s7_is_symbol(s7_car(args))) {
        ...
        pd_error(
            (t_object*)x,
            "s4pd: (table-read): arg 1 must be a symbol of an array name");
        return s7_return_value;
    }
    ...
    if (!(array = (t_garray*)pd_findbyclass(gensym(array_name),
                                            garray_class))) {
...
static s7_pointer s7_table_write(s7_scheme* s7, s7_pointer args)
{
    ...
    if (s7_is_symbol(s7_car(args))) {
        ...
        pd_error(
            (t_object*)x,
            "s4pd: (table-write): arg 1 must be a symbol of an array name");
        return s7_return_value;
    }
    if (s7_is_number(s7_cadr(args))) {
...
static s7_pointer s7_pd_output(s7_scheme* s7, s7_pointer args)
{
    ...
    // all added functions have this form, args is a list, s7_car(args) is the
    // first arg, etc
    ...
    if (s7_is_real(s7_out_val) || s7_is_boolean(s7_out_val)) {
        ...
        if (err) {
            post("s4pd: error outputing %s",
                 s7_object_to_c_string(s7, s7_out_val));
        } else {
            outlet_anything(x->outlets[outlet_num], gensym("float"), 1,
                            &output_atom);
        }
    }
    // symbols, keywords, chars, and strings all become pd symbols
    // in PD, symbol messages always start with 'symbol', unlike Max, where the
    // message type IS the symbol
    else if (s7_is_string(s7_out_val) || s7_is_symbol(s7_out_val)
             || s7_is_character(s7_out_val)) {
        ...
        if (err) {
            ...
            // note that unlike Max, the output message is always "symbol
            // {thing}"
            outlet_anything(x->outlets[outlet_num], gensym("symbol"), 1,
                            &output_atom);
        }
    }
    // lists
    else if (s7_is_list(s7, s7_out_val) && !s7_is_null(s7, s7_out_val)) {
        // array of atoms to output, we overallocate for now rather than do
        // dynamic allocation
        t_atom out_list[MAX_ATOMS_PER_OUTPUT_LIST];
        s7_pointer* first = s7_car(s7_out_val);
        int length = s7_list_length(s7, s7_out_val);

        // lists have have two cases: start with symbol or start with
        // number/bool
        if (s7_is_number(first) || s7_is_boolean(first)) {
            // post("outputting list with numeric or bool first arg, becomes
            // 'list' message");
            for (int i = 0; i < length; i++) {
                s7_obj_to_atom(s7, s7_list_ref(s7, s7_out_val, i),
                               &out_list[i]);
            }
            // add the symbol "list" as the first item for the message output
            outlet_anything(x->outlets[outlet_num], gensym("list"), length,
                            out_list);
        } else {
            ...
            // thus to output a 'list' message, user can do (out 0 '(list :a :b
            // :c))
            err = s7_obj_to_atom(s7, first, &output_atom);
            outlet_anything(x->outlets[outlet_num],
                            atom_getsymbol(&output_atom), length - 1,
                            out_list);
        }
    }
    ...
    // TODO: should support output of vectors of symbols too I think...
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (s7_is_number(item) || s7_is_boolean(item)) {
                ...
                pd_error(0,
                         "s4pd: Vector output only supported for ints, "
                         "floats, & booleans");
                return s7_nil(s7);
            }
        }
    ...
    // unhandled output type, post an error
        ...
        pd_error(0, "s4pd: Unhandled output type %s",
                 s7_object_to_c_string(s7, s7_out_val));
    }
...
{
    // post("s4pd_s7_call()");
    // post(" - function: %s args: %s", s7_object_to_c_string(x->s7, funct),
    // s7_object_to_c_string(x->s7, args));
    int gc_loc;
    s7_pointer old_port;
    const char* errmsg = NULL;
    char* msg = NULL;
...
void s4pd_load_from_path(t_s4pd* x, const char* filename)
{
    ...
    // use canvas_open to get full path, but then don't load from the
    // descriptor
    int filedesc;
    char path_buf[MAXPDSTRING], *name_buf;
    ...
    if ((filedesc = canvas_open(x->x_canvas, filename, "", path_buf, &name_buf,
                                MAXPDSTRING, 0))
        < 0) {
        pd_error(0, "s4pd: Can't find file %s. (Check Pd file paths)",
                 filename);
        return;
    }
    close(filedesc);
...
void s4pd_s7_load(t_s4pd* x, char* full_path)
{
    ...
    if (msg) {
        ...
        // we don't run this in production as the res printed is the last line
        // of the file loaded, which looks weird to the user
    }
}
...
static s7_pointer s7_schedule_delay(s7_scheme* s7, s7_pointer args)
{
    ...
    // post("s7_schedule_delay() time: %5.2f handle: '%s'", delay_time,
    // cb_handle_str);

    //  allocate memory for the clock_info struct, holds gensym handle, ref to
    //  s4pd, and ref to clock
    // NB: this gets freed by clock callback after clock fires
    t_s4pd_clock_info* clock_info = (t_s4pd_clock_info*)getbytes(
        sizeof(t_s4pd_clock_info));
    clock_info->obj = (void*)x;
    clock_info->handle = gensym(cb_handle_str);
    // make a clock, setting our clock_info struct as the owner, as void
    // pointer when the callback method fires, it will retrieve this pointer as
    // an arg and use it to get the handle for calling into scheme
    t_clock* clock = clock_new((void*)clock_info,
                               (t_method)s4pd_clock_callback);
    ...
    // return the handle on success so that scheme code can save it for
    // possibly cancelling later
    return s7_make_symbol(s7, cb_handle_str);
}

// the callback that runs for any clock and is used to find the delayed
// function in Scheme
void s4pd_clock_callback(void* arg)
{
    ...
    // call into scheme with the handle, where scheme will call the registered
    // delayed function
    s7_pointer* s7_args = s7_nil(x->s7);
...
// NOT IN USE RIGHT NOW - delays just harmlessly fire off and Scheme does
// nothing might bring this back later with the new queue though
/*
static s7_pointer s7_cancel_delay(s7_scheme *s7, s7_pointer args){
    ...
    // pd(99452,0x10cf1e5c0) malloc: *** error for object 0x106400110: pointer
being freed was not allocated clock_unset(clock_info_ptr->clock);
